...
int main()
{
    ...
    // read users.txt and store in users and passwords arrays
    ...
    // socket initialization and binding to port
    ...
    // get the path of the current working directory and store in directory array
    ...
    // accept() connections and add to fd_set
    printf("Listening....\n");
    while (1)
    {
        ...
        // for each client, check if it is ready to establish a connection
        for (int fd = 0; fd <= max_fd; fd++)
        {
            if (FD_ISSET(fd, &ready_fdset))
            {
                ...
                {
                    if (initiate_communication(fd, &authen_fdset, &user_fdset, num_users) == -1)
                    {
